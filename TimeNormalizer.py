...
class TimeNormalizer:
    ...
    def __restore(self, text, target):
        replace_dict = {'中旬':'15号','傍晚':'午后','五一': '劳动节','白天':'早上'}
        replace_word = ['的',' ']
        if len(text):
            new_temp = []
            for key,value in replace_dict.items():
                if value in self.target and key in target:
                    for i in text:
                        if value in i:
                            new_temp.append(i.replace(value, key))
                        else:
                            new_temp.append(i)
    ...
    def init(self):
        ...
        # except:
        #     with open(os.path.dirname(__file__) + '/resource/regex.txt', 'r', encoding="utf8") as f:
        #         content = f.read()
    ...
    def parse(self, target, timeBase=arrow.now()):
        ...
        self.__preHandling()
        self.timeToken, temp = self.timeEx(target)

        # print('temp is {}'.format(temp))
        dic = {}
        res = self.timeToken
        # for ele in res:
        #     print('ele.tp_origin.tunit is {}'.format(ele.tp_origin.tunit))
        ...
        if self.isTimeSpan:
            ...
            if len(res) == 0:
                ...
                if res[0].time > res[1].time:
                    new_time = res[1].time.shift(days=1)
                    dic['timespan'] = [res[0].time.format("YYYY-MM-DD HH:mm:ss"), new_time.format("YYYY-MM-DD HH:mm:ss")]
                else:
                    dic['timespan'] = [res[0].time.format("YYYY-MM-DD HH:mm:ss"), res[1].time.format("YYYY-MM-DD HH:mm:ss")]
                dic["text"] = [temp[0], temp[1]]
        return dic
    ...
    def timeEx(self, target):
        ...
        # todo
        contextTp = TimePoint()
        # print('timeBase is {}'.format(self.timeBase))
        # print('temp is {}'.format(temp))
        ...
        temp = self.__restore(temp, target)
        return res, temp
